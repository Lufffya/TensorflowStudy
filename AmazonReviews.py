...
def Get_Encoding(inputs):
    ...
    for i in tqdm(range(0,len(inputs),32)):
        ...
        batch_word_embedding = bert_layer(batch)

        #batch * 128 * 768
        for _batch in batch_word_embedding:
            #1 * 128 * 768
            array = []
            for j in range(768):
                #128 * 1
                temp = 0
                for k in range(len(_batch)):
                    temp += _batch[k][j]
                array.append(temp/(len(_batch)))            
            outPut.append(array)
...
for _index, distance in _sorted[0:50]:
...
#     csv_File = open('DataSet\新建文本文档{0}.csv'.format(_label),'w',encoding='utf-8')
#     csv_writer = csv.writer(csv_File)
#     for review in classResult:
